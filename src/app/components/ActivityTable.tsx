...
export default function ActivityTable() {
  ...
  function SortIcon({ col }: { col: SortKey }) {
    if (sort.key !== col) return <ArrowUpDown className="inline ml-1 w-3 h-3 opacity-30" />;
    return sort.dir === 'asc'
  ...
  return (
    ...
    >
      ...
      <div className="flex rounded-xl border border-border overflow-hidden bg-card shadow-sm">
        ...
        <div
          className={cn(
            'hidden md:flex w-72 shrink-0 transition-all duration-200',
            panelVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-3 pointer-events-none'
          )}
        >
